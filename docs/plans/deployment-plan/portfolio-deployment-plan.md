...
| 4   | Vercel **Ignored Build Step** set to a bash diff                       | Skips the frontend build when nothing changed in `apps/frontend`, `package.json`, or `package-lock.json`, saving Hobby build quota (100 builds/day) on backend-only pushes. Edge case: `$VERCEL_GIT_PREVIOUS_SHA` is empty on the very first deploy, so the diff errors - but an error makes Vercel build anyway (fails safe), i.e. one extra build at the start, then skips forever after. | Vercel dashboard → project settings → Git → Ignored Build Step |

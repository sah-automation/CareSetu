...
- **Horizontal scaling runbook + worker count tuning** - single uvicorn instance + PaaS-native (Render) today; the Dockerfile (7.2.8) is the seed. Needs an orchestration decision (single VM same-origin edge path, per ADR-0007) before any multi-instance work. Note: horizontal scale-out and worker-count tuning apply to **API replicas only** - the outbox poll loop stays a single process by design (exactly-one-worker rule in `docs/plans/deployment-plan/worker-outbox-runbook.md`, ticket #394).

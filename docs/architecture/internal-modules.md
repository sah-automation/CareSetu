...
- **Brute-force lockout (a counter, never identity state):** 10 consecutive verification failures across challenges → 15-minute temporary phone lockout, carried as the `lockout_failed_attempts` + `lockout_until` columns on `iam_identities` (never as the identity lifecycle status), enforced on both `verify_otp` and `resend_otp`, reset by a successful verification or by the next failure once the window has fully elapsed (a failure after the lockout lifts starts a fresh streak). Distinct from the `Suspended` identity status, which stays reachable only via `set_actor_status` (Phase 5).

...
- **Partner phone-OTP login (FEAT-014, ADR-0016):** runs on dedicated `POST /v1/auth/partner/*` routes that reuse the unchanged OTP challenge machine; the session mint requires a phone-verified identity, and renewal stays open for `[Registered]` / `[Under Verification]` / `[Rejected]` partners, refused only for suspended identities. The pre-activation restriction is enforced by the partner self-service routes, not a distinct login kind, and no new event name is registered - OTP issuance reuses `otp.sent` and partner phone verification is silent.
...
- **Inbound Sync APIs:** Case-console facade (`CaseConsoleFacade` - #432/#434): `mark_consult_complete(doctor, case_id)`, `get_case(doctor, case_id)`, `list_doctor_cases(doctor)`, `submit_doctor_input(doctor, case_id, input_type, media_ref)`, `close_case_without_rx(doctor, case_id, close_reason)` (terminal `[Case: Closed]`, publishes `case.closed`). Prescription facade (`PrescriptionFacade`): `create_rx_draft(case_id, doctor, source, items)` (drafting cap: two AI drafts per care case), `save_rx_revision(case_id, rx_id, doctor, rx_items)`, `approve_prescription(case_id, rx_id, doctor, verification_declaration)`, `reject_prescription(case_id, rx_id, doctor, reason)`, `get_approved_prescription(rx_id, doctor)` (doctor-scoped - a foreign doctor reads not-found).
- **Inbound Events Subscribed:** `pre_summary.ready` (real case birth - the review-close consumer writes the care case in the same transaction as the consumed-event mark), `pre_summary.low_confidence` (record-and-surface `forced_review` flag on the matching case), `report.filed` (case-context seam, Phase 9).
...
- **Event Wiring (PHASE-8 T08 #424 + review-close #428):** `register_handlers` in `care/adapters/__init__.py` owns the producer payload models for **all seven** case/rx events care publishes, incl. `prescription.issued` (the producer owns its registry contract; MOD-003 consumes it through its own tolerant mirror, so the row payload is reconstructed with a typed model before fan-out) and subscribes to the three inbound events as ledgered seams. The case-birth write for `pre_summary.ready` consumes the producer-carried patient identity - the payload is enriched with `patient_id` (intake producer + care's tolerant consumer mirror, review-close #428); a legacy in-flight `pre_summary.ready` without patient identity degrades to the telemetry-only log instead of crashing.
...
- **Constraint:** Draft comes from `MOD-005` (`request_rx_draft` seam, drafting cap: two AI drafts per care case); issuance authority stays with the licensed doctor (ADR-0015, baseline `CFL-002`/`RISK-EVAL-003` resolved).
...
| `MOD-006` (Care)           | `MOD-005` (Intake)              | Internal API                   | DraftDTO                                                                                                                                | `request_rx_draft` - AI drafting-assistant seam (drafting cap: two AI drafts per case)               | `FEAT-009`                    |
...
| `MOD-008` (Fulfillment)    | `MOD-006` (Care)                | Internal API                   | RxDTO (items, patient)                                                                                                                  | `get_approved_prescription(doctor, rx_id)` - doctor-scoped routing source of truth (Phase 10 caller) | `FEAT-012`                    |
...
| `Patient` (channel)        | `MOD-004` (Consent)             | Internal API                   | PickDTO                                                                                                                                 | Doctor pick + consent grant (assigned-partner scoping)                                               | `FEAT-002`, `NFR-SEC-006`     |
| `Doctor` (channel)         | `MOD-005` (Intake)              | Internal API                   | ReviewQueueDTO                                                                                                                          | Doctor review-queue read (assigned pre-summaries, low-confidence first)                              | `FEAT-007`                    |
| `Doctor` (channel)         | `MOD-005` (Intake)              | Internal API                   | PreSummaryDetailDTO                                                                                                                     | Doctor full pre-summary read (structured summary, symptoms, confidence, review state)                | `FEAT-007`                    |
| `Doctor` (channel)         | `MOD-006` (Care)                | Internal API                   | WorkingPrescriptionDTO                                                                                                                  | Working prescription read (current in-progress rx revision for a case)                               | `FEAT-009`                    |
| `Doctor` (channel)         | `MOD-002` (Partner)             | Internal API                   | PartnerProfileDTO                                                                                                                       | Consultation fee on directory and provider-profile projections (settable by doctor)                  | `FEAT-004`, `FEAT-005`        |
...
| `pre_summary.ready`                            | `MOD-005` (Intake)          | `MOD-006` (care-case birth - patient identity on payload), `MOD-010` (in-app notify), `MOD-011` | JSON           | At-least-once              |
| `pre_summary.low_confidence`                   | `MOD-005` (Intake)          | `MOD-006` (record-and-surface `forced_review` flag), `MOD-011`                                  | JSON           | At-least-once              |

...
4. **Lockout is a counter, never identity state.** Ten consecutive verification failures across challenges trigger a 15-minute temporary phone lockout, implemented as the `lockout_failed_attempts` + `lockout_until` counter columns on `iam_identities` - never as the identity lifecycle status - enforced on both `verify_otp` and `resend_otp`, and reset only by a successful verification.

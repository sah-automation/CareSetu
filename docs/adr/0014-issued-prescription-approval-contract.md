...
- **CFL-002 seam landed (review-close #430/#432):** the verification-declaration gate is the documented replaceable compliance seam - `PrescriptionFacade._check_approval_declaration` sits at the facade as the gate a stricter rule slots into, and the approval flow records the declaration on `care_rx_approvals` (`verification_declaration` boolean + `declared_at`), with the prescription machine's own declaration guard kept as defense-in-depth. A stricter regulatory rule (e.g. human-in-the-loop sign-off) slots into this facade gate without redesign.

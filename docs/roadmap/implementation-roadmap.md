...
PHASE-8.1 Doctor console UI + pick-a-doctor (8.1) ◄─ Phase-8 (backend seams), Phase-6 (directory)
...
| **`PHASE-8.1`** | Doctor Console UI & Pick-a-Doctor               | Frontend chassis insert: patient pick-a-doctor step (filtered directory, consent-at-pick, `MOD-004` atomic grant), doctor console (review queue, case workspace, prescription workflow), six backend deltas (pick write, queue read, pre-summary read, working rx read, low-confidence finalize, consultation fee) (`FEAT-008`, `FEAT-009` frontend)                           | Pick-a-doctor → doctor console → approved e-prescription |
...
**Delivery status (2026-09-15):** `PHASE-0`, `PHASE-1`, `PHASE-2`, and the two chassis inserts `PHASE-2.5` (#146) / `PHASE-2.6` (spec #191) are built, verified, and deployed to the split-origin demo (Vercel frontend + Render backend - see `ADR-0007`). `PHASE-8` (MOD-006 care case + e-prescription) is delivered on the backend - see its §2.8 status; the doctor console UI and pick-a-doctor flow are delivered as the chassis insert `PHASE-8.1` (see §2.8a). All other phases stay as the §3.1 delivery matrix records. Canonical written form for the insert IDs is dotted (`PHASE-2.5`, `PHASE-2.6`, `PHASE-8.1`); ticket brief folders spell them dash-style (`PHASE-2-5-*`, `PHASE-2-6-*`, `PHASE-8-1-*`) for filesystem safety.
...
No new PRD feature - frontend chassis insert delivering the doctor console UI and patient pick-a-doctor step, anchored to `FEAT-008` (consult handshake) and `FEAT-009` (e-prescription) frontend surfaces, plus six backend deltas that close remaining read/write gaps.

| PRD Anchor              | Scope                                                                                     | Internal Module ID                                | External Interface ID                   |
| :---------------------- | :---------------------------------------------------------------------------------------- | :------------------------------------------------ | :-------------------------------------- |
| `FEAT-008` / `FEAT-009` | Pick-a-doctor step, doctor console (queue + case workspace + rx workflow), backend deltas | `MOD-006` (additive), `MOD-004` (consent-at-pick) | `ACT-001` (Patient), `ACT-002` (Doctor) |

#### 2. Deferred / Out-of-Scope Items

- Patient-facing case status page (deferred to Phase 14 channel work).
- Doctor "Patients" and "Profile" console tabs (visible only as "coming soon").
- Ratings, reviews, slots, availability calendars, appointment booking.
- Fee tiers, insurance handling, consultation billing.
- Browser end-to-end (Playwright) coverage for these flows - unit and route seams only.
- Any change to the consented-history read itself; the console only consumes the existing read.
...
- **Backend deltas:**
  ...
  - Doctor pre-summary read: doctor-scoped read returning full pre-summary content (structured summary, symptoms, confidence flag, review state) to an assigned doctor.
  - Low-confidence one-action finalize: review action changes so a doctor's attributed review of a low-confidence pre-summary reaches `final` in the same action instead of stopping at `reviewed`.
  - Working prescription read: doctor-scoped read returning the current in-progress prescription revision for a case (draft, reviewed, or rejected).
  - Consultation fee: nullable fee added to partner profile, exposed on directory entry and verified-safe provider profile projection, settable by the doctor through a partner-scoped update endpoint.
- **Migration Scripts:** none.

#### 4. Infrastructure, DevOps & Environment Targets

- Doctor channel (ACT-002) route group in the shared Next.js deploy; intake client gains queue read, pre-summary read, and patient pick write.
- Care-plan API client module added alongside existing per-module clients; every care mutation sends the existing idempotency-key contract header.
- i18n: new copy in shared bilingual dictionary under a new `doctorConsole` block plus pick-step additions; en/hi parity test covers new strings.

#### 5. Phase Dependency & Risk Matrix

| Dependency / Blocked By       | Potential Risk                                                      | Mitigation Plan                                                                          |
| :---------------------------- | :------------------------------------------------------------------ | :--------------------------------------------------------------------------------------- |
| `PHASE-8` (backend seams)     | Pick-a-doctor depends on delivered consent and case-birth seams     | Six backend deltas close remaining gaps; existing seams consumed, not replaced           |
| `PHASE-6` (directory search)  | Pick-a-doctor reuses the verified directory; directory must be live | Pick screen filters over the existing verified-only directory entry projection           |
| `MOD-004` (consent lifecycle) | Consent-at-pick must be atomic with the doctor choice               | Single transaction records pick + consent grant; existing `check_consent` gate unchanged |
...
- [x] **Every `FEAT-001`–`FEAT-020` assigned to exactly one phase** (§3.1), with chassis-insert extensions where a feature's frontend ships separately after its backend (`PHASE-8.1` extends `FEAT-008`/`FEAT-009`).

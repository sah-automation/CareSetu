...
class MockWhatsAppChannel:
    ...
    async def send(self, request: DeliveryRequest) -> DeliveryResult:
        ...
        return DeliveryResult(request_id=f"mock-wa-{secrets.token_hex(8)}", status="queued")
        ...
        for attempt in range(self._max_retries + 1):
            ...
            try:
                ...
                if attempt == self._max_retries:
                    logger.error(
                        "EXT-003 WhatsApp send failed after %d attempts "
                        "(network error) for phone %s",
                        self._max_retries + 1,
                        mask_phone(request.recipient_phone_e164),
                    )
                    raise NotificationDeliveryError(
                        "EXT-003 WhatsApp send failed after "
            ...
            if response.is_success:
                return _parse_whatsapp_response(response)
            logger.warning(
                "EXT-003 WhatsApp send rejected with HTTP %d for phone %s",
                response.status_code,
                mask_phone(request.recipient_phone_e164),
            )
            raise NotificationDeliveryError(
                f"EXT-003 WhatsApp send rejected with HTTP {response.status_code}",
                retries_exhausted=False,
            )
        logger.error(
            "EXT-003 WhatsApp send failed after %d attempts (last HTTP %d) for phone %s",
            self._max_retries + 1,
            last_status,
            mask_phone(request.recipient_phone_e164),
        )
...
def _parse_whatsapp_response(response: httpx.Response) -> DeliveryResult:
    try:
        payload = response.json()
    except ValueError as exc:
        raise NotificationDeliveryError(
            "EXT-003 WhatsApp returned a non-JSON response", retries_exhausted=False
        ) from exc
    if not isinstance(payload, dict):
        raise NotificationDeliveryError(
            "EXT-003 WhatsApp returned an unexpected response payload",
            retries_exhausted=False,
        )
    try:
        ...
        raise NotificationDeliveryError(
            "EXT-003 WhatsApp returned an invalid response payload "

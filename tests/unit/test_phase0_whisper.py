...
def test_parse_chat_content_redacts_error_message_keeps_metadata() -> None:
    response = {"error": {"type": "invalid_request_error", "code": "invalid_api_key"}}
    with pytest.raises(RuntimeError, match="invalid_api_key") as excinfo:
        parse_chat_content(response)
    assert "Invalid API key provided" not in str(excinfo.value)

...
async def test_rate_limit_counts_intake_write_surface_only() -> None:
    ...
    ``upload-media``, ``submit``, the ``{intake_id}/pick-doctor`` shape (#443),
    and the ``{intake_id}/re-record`` shape - each counted into the intake
    surface's own per-IP bucket. Reading an intake back (detail, pre-summary,
    clip playback) is never counted and never capped.
    """
    middleware = RateLimitMiddleware(app=None, enabled=True, max_requests=4, window_seconds=60)

"""PHASE-1 T5a: module scaffold batch 1 - hexagonal layout + namespace prefixes (#24).

Asserts the generator can emit a module package into a throwaway directory and
that the six batch-1 modules are scaffolded under ``apps/backend/modules/``
with the hexagonal layout from coding-standards §2 (``domain/``, ``adapters/``,
...
BATCH_1 = ("iam", "partner", "health", "consent", "intake", "care")
...
def test_batch1_modules_scaffolded() -> None:
    for name in BATCH_1:
